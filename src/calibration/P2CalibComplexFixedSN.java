...
/** 
 * P2CalibComplexFixedSN class
 * 
 * Class that extends Parameters2Calib in order to specify the parameters
 * for calibration. In this case, we have a fixed static SN (specify in the config file)
 * and then 3 parameters if just one segment (1 integer for social adoption and 2 reals)
 * 
 ...
 * 
 */
public class P2CalibComplexFixedSN extends Parameters2Calib {
	...
	public P2CalibComplexFixedSN(int _numParameters2Calib) {
	...
	public void specifyParams (double[] _params){
		...
		for (int k = 0; k < this.getNumSegments(); k++) {
			...
			// THIS PARAMETERS HAS NO EFFECT: -1 
			setSegmentDailyProbObtainSubscription(k,  -1);
