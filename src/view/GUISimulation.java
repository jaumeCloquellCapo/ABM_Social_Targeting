...
/**
 * A GUI class, draws also a time serie chart. 
 * The code is from MASON "howto.html" -> "How to Add a Chart Programmatically"
 * 
 *  @author mchica
 *  
 */
public class GUISimulation extends GUIState {
	...
	public void start() {
        ...
        series = new XYSeries[1];    // cumulative
        series2 = new XYSeries[2];   // new and historical data
        ...
        scheduleRepeatingImmediatelyAfter(new Steppable() {
			...
			public void step(SimState state) {
               ...
               // y is the current number of premiums users for the first chart
               y1[0] = model.getCumPremiumsAtStep(x[0]);  
               
               // for the second chart there are two: the historical data and the new users
               //y2[0] = model.getNewPremiumsAtStep(x[0]);  
               y2[0] =  model.getNewPremiumsAtStep(x[0]) * model.getParametersObject().getRatioAgentsPop();
               
               if (x[0] < data.getNumSteps()) {
            	   y2[1] =  data.getHistoricalDataArray(x[0]);
                   	   //	(int) Math.round(data.getHistoricalData(x[0])/model.getRatioAgentsPop());
